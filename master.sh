...
fetch_scripts() {
    scripts_recursive=$(curl -s "https://api.github.com/repos/seanssmith/Script-Repo/git/trees/$BRANCH?recursive=1" | jq -r '.tree[] | select(.type == "blob" and .path | endswith(".sh")) | .path')
    scripts_root=$(curl -s "https://api.github.com/repos/seanssmith/Script-Repo/contents?ref=$BRANCH" | jq -r '.[] | select(.type == "file" and .name | endswith(".sh")) | .path')
